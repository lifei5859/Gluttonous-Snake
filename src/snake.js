...
snake.init = function (ground){
    let oSnakeHead =  Square.cerate('SnakeHead',3,1,'url(./img/snake.png)')
    let oSnakeBody1 =  Square.cerate('SnakeBody',2,1,'url(./img/snake.png)')
    let oSnakeBody2 =  Square.cerate('SnakeBody',1,1,'url(./img/snake.png)')
...
snake.strategies = {
    move(snake,site,ground,key){
         let newBody = Square.cerate('SnakeBody',snake.head.x,snake.head.y,'url(./img/snake.png)')
         newBody.next = snake.head.next
         newBody.next.last = newBody
         newBody.last = null
         ground.remove(snake.head.x,snake.head.y)
         ground.add(newBody)
         let newHead = Square.cerate('SnakeHead',site.x,site.y,'url(./img/snake.png)')
         ...
         ground.add(newHead)
        if(!(key=='eat')){
            let newFloor = Square.cerate('Floor',snake.tail.x,snake.tail.y,'url(./img/map.jpg)')
    ...
    transfer(snake,site,ground){
        ...
        setTimeout(function(){
            game.transferArr.forEach(ele=>{
                ...
                let newFloor=Square.cerate('Floor',ele.x,ele.y,'url(./img/map.jpg)')
                ground.add(newFloor)
                }
            )
            game.transferArr=[]
